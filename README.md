...
```bash
sudo mkdir -p /srv/bizeros/brain

docker run -d \
  --name bizeros \
  --restart unless-stopped \
  -v /srv/bizeros/brain:/brain \
  -p 127.0.0.1:8080:80 \
...
- **`.agents/skills/bizeros-knowledge`** — agent skill (agentskills.io format) compatible with Hermes Agent, OpenClaw, and Claude Code; ClawHub-publishable
...
# OpenClaw
ln -s /opt/bizeros-knowledge/.agents/skills/bizeros-knowledge ~/.openclaw/skills/bizeros-knowledge

# Set the brain dir for the agent process
export BRAIN_DIR=/srv/bizeros/brain
```

The skill exposes four tools to the agent: `list_notes`, `search_notes`, `read_note`, `write_note`. See `.agents/skills/bizeros-knowledge/SKILL.md` for the conventions and tool reference the agent follows.
